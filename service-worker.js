...
const CACHE_VERSION = 'v17'; // v17: サイドバー関連ツール一覧に「再訪精霊データベース」を
// 羽トラッカーの直後へ追加したための強制キャッシュ更新
// v16: ダッシュボードカレンダーのイベント配色をかわいらしい
// パステル系に変更（背景色ごとにWCAG相対輝度から白文字/黒文字を自動選択）したための
// 強制キャッシュ更新
// v15: 連鎖オーバーレイ切替時（A→Bの自動排他close、および
// プロフィール/表示設定内の専用ボタンによる明示的なclose→open連鎖）でフォーカストラップの
// 復元先を誤る不具合を修正（サプレッションフラグ＋非フォーカス可能時のドックへのフォールバック）
// したための強制キャッシュ更新
// v14: ダッシュボードカレンダーの開始日バーも、開始時刻が日中
// (16:00等)の予定については半分幅（右詰め）に短縮するよう改善（v13の最終日側と対称）。
// v13: ダッシュボードカレンダーの最終日バーを、終了時刻が日中
// (15:59等)の予定については半分幅に短縮するよう改善。
// v12: 「?」ショートカットがデータ管理/比較/アイコンカスタムの
// 各サブモーダルを閉じずに表示設定を重ねて開いてしまう不具合、isTyping判定がINPUT要素のtypeを
// 見ておらずチェックボックス/カラー等にフォーカスがあるだけで「?」「d」ショートカットが
// 効かなくなっていた不具合、オーバーレイ表示中に下部ドックのz-indexがオーバーレイ背景より
// 低くクリックを奪われていた不具合を修正したための強制キャッシュ更新
// v11: サイドバードロワーのフォーカストラップが機能していなかった不具合
// （setup()の返り値からsidebarElが漏れていた）、ダッシュボード初回オープン時に非同期データ取得と
// 初期フォーカスが競合していた不具合、フォーカストラップがsummary要素を対象にしていなかった不具合
// を修正したための強制キャッシュ更新
// v10: イベントカレンダーで、由来の異なる複数の再訪精霊スケジュールが
// 同時に表示された際に同じ色へ衝突していた不具合を修正（色割り当てキーをスケジュール単位に分離）
// したための強制キャッシュ更新
// v9: モーダル幅をitem基準の720/960px(850px以上)へ統一し、
// フォーカストラップをwings同様のスタック方式へ刷新、トップレベルオーバーレイの排他制御を追加
// したための強制キャッシュ更新
// v8: イベントカレンダーを日ごとの丸ドット＋凡例からGoogleカレンダー風の
